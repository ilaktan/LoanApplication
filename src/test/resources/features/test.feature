...
Feature: Personal Loan
  ...
  Scenario Outline: as a user over 18 (Happy path)
  ...
  Scenario Outline: as a user under 18 (Negative)
    Given User navigate the loan page
    Then User enter the borrow of amount
    Then User enter the months
    Then User enter the "<reason>" for loan
    ...
    Then user enter address info
    And User shouldn't be able to see "Thank you for your application" message
    Examples:
      | reason             |
      | Car finance        |
      | Debt consoildation |
